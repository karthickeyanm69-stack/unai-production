...
import { Smartphone, Check, Delete, ShieldCheck } from 'lucide-react';
...
export const MakePaymentPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F5EF] py-8 sm:py-12 px-4 sm:px-6 flex items-center justify-center relative overflow-hidden text-[#1E2732]">
        ...
        <div className="flex items-center justify-center space-x-2 sm:space-x-3">
          ...
          {[1, 2, 3].map((mult) => (
            <button
              ...
              className={`px-3.5 py-2 rounded-[14px] font-mono text-xs font-bold transition-all border ${
                multiplier === mult
        ...
        <div className="grid grid-cols-3 gap-2.5 sm:gap-3 py-1 sm:py-2">
          {['1', '2', '3', '4', '5', '6', '7', '8', '9', 'CLEAR', '0', 'DEL'].map((key) => (
            <button
              ...
              className="h-12 sm:h-14 bg-slate-100 hover:bg-slate-200 active:scale-95 text-[#1E2732] font-['Sora'] font-extrabold text-lg sm:text-xl rounded-[14px] border border-slate-200 transition-all flex items-center justify-center shadow-sm"
            >
              {key === 'DEL' ? (
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 bg-[#2E9E5B] flex flex-col items-center justify-center p-6 text-white text-center animate-fade-up">
          ...
          <button
            ...
            className="w-full max-w-xs py-3.5 sm:py-4 bg-white text-[#2E9E5B] font-['Sora'] font-extrabold text-xs sm:text-sm rounded-[14px] shadow-2xl hover:bg-emerald-50 transition-all"
          >
